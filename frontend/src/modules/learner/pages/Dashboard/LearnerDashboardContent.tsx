/**
 * Learner Dashboard Content — layout-free
 * All dashboard sections without LearnerLayout wrapper.
 * Used by:
 *  - Learner Dashboard page (wrapped in LearnerLayout)
 *  - Trainer Dashboard "View Portfolio" inline embed
 */
...
const getInfoCardIcon = (iconType?: string) => {
  switch (iconType) {
    case 'lightbulb': return <LightbulbIcon sx={{ fontSize: 28, color: '#1E1E2D' }} />;
    case 'clipboard': return <ClipboardIcon sx={{ fontSize: 28, color: '#1E1E2D' }} />;
    case 'barchart':  return <BarChartIcon  sx={{ fontSize: 28, color: '#1E1E2D' }} />;
    default:          return null;
  }
};
...
const WelcomeBanner: React.FC = () => (
  <WelcomeBannerWrapper>
    ...
    <SafeguardingRow>
      ...
      <SafeguardingLink href="mailto:info@primelearning.uk">Email: info@primelearning.uk</SafeguardingLink>
      <SafeguardingLink href="tel:+4409988423">Phone: +4409988423</SafeguardingLink>
    </SafeguardingRow>
  </WelcomeBannerWrapper>
);

// ─── Section 2: Tabs & Trainer Card ─────────────────────────────────────────
    ...
    <StatisticsCardWrapper>
      ...
      <StatCardWrapper>
        ...
        <DonutChartWrapper>
          ...
          <LegendWrapper>
            <LegendItem><LegendDot color="#4CAF50" /><span>Progress {totalProgress}%</span></LegendItem>
            <LegendItem><LegendDot color="#E8E8E8" /><span>Due {100 - totalProgress}%</span></LegendItem>
          </LegendWrapper>
        </DonutChartWrapper>
      </StatCardWrapper>
      ...
      <StatCardWrapper>
        ...
        <CalendarWrapper onClick={() => router.push('/learner-dashboard/calendar')}>
          <CalendarHeader>
            <CalendarMonthDisplay>{MONTH_NAMES[state.calendarMonth]} {state.calendarYear}</CalendarMonthDisplay>
            <Box sx={{ display: 'flex', gap: '2px' }}>
              <CalendarNavButton onClick={(e) => { e.stopPropagation(); previousMonth(); }}>
                <NavPrevIcon sx={{ fontSize: '18px' }} />
              </CalendarNavButton>
              <CalendarNavButton onClick={(e) => { e.stopPropagation(); nextMonth(); }}>
          ...
          <TaskDueLegend>
            {state.tasksDue.map((task) => (
              <TaskDueLegendItem key={task.category}>
                ...
                <span>{task.category}: {task.count}</span>
              </TaskDueLegendItem>
            ))}
          </TaskDueLegend>
        </TaskDueWrapper>
...
// ─── Section 6: Learning Aims & Information Options ───────────────────────────
              ...
              {state.learningAims.map((aim) => (
                <TableRow key={aim.id}>
                  ...
                  <TableCell align="center">
                    <ActionChevron><ExpandMoreIcon /></ActionChevron>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          ...
          <InformationOptionsList>
            {state.informationOptions.map((option) => (
              ...
              >
                ...
                <InformationOptionChevron><ChevronRightIcon /></InformationOptionChevron>
              </InformationOptionItem>
            ))}
          </InformationOptionsList>
        </InformationOptionsWrapper>
